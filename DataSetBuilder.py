...
class gp_builder:
    def __init__(self,str='coco',data_files=[]):
        self.data_files = ['train2014.zip', 'val2014.zip', 'annotations_trainval2014.zip']
        self.train_dataset = []
        self.val_dataset=[]
        ...
        while True:
            ...
            for i in range(batch_size):
                ...
                for obj in processed_objs:
                    ...
                    proid[i, obj[5], :, :] = [[1.] * classes] * anchors #Bullshit - ملهاش لازمه
    ...
    def set_data(self):
        data_files = []
        urls = ['http://images.cocodataset.org/zips/train2014.zip','http://images.cocodataset.org/zips/val2014.zip','http://images.cocodataset.org/annotations/annotations_trainval2014.zip']
        for url in tqdm(urls):
          wget.download(url)
          data_files.append(url.split('/')[-1])
          
          with zipfile.ZipFile('/content/'+data_files[-1], 'r') as zipped:
            zipped.extractall('/content')
            os.unlink(data_files[-1]) 
    ...
    def get_andata(self):
        ...
        annotations_dirs = [self.data_files[2]+'/instances_train2014.json',self.data_files[2]+'/instances_val2014.json']
        train_andata = dict()
        val_andata = dict()
    ...
    def prepare(self):
        ...
        for key in sorted(vimgs.keys()):
            try:
                self.val_dataset.append([vimgs[key],np.array(vimg_bbox[key])])
            except : continue
    ...
    def report(self,timgs,timg_bbox):
        ...
        print('length of downloaded images : {}'.format(len(os.listdir('/content/train2014'))))

    def _get_data(self):
        return np.array(self.train_dataset),np.array(self.val_dataset)
